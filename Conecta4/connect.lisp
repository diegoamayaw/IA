...
(defun max-value (nivel player estado alfa beta)
	...
	(setq v -1000)
	(setq movPosibles (movPos estado player))
	(agregaAestados estado (heuristica estado));agrega el estado con su respectivo costo
	
		(loop for x in movPosibles do 
			(cond
				((equal nivel 0) v
				)
			(t(setq vPrim (min-value (- nivel 1) 'X x (heuristica estado) beta))
...
(defun min-value (nivel player estado alfa beta)
	...
	(agregaAestados estado (heuristica estado));agrega el estado con su respectivo costo
		...
		(loop for x in movPosibles do 
			...
			(t(setq vPrim (max-value (- nivel 1) 'O x alfa (heuristica estado)))
			(if (< vPrim v) (setq v vPrim))
...
;Tabla hash, llave --> estado, contenido --> costo
(setq costosEstados (make-hash-table :size 50))

(defun agregaAestados (estado costo)
	(setf (gethash estado costosEstados) costo)
)
;(load "C:\\Users\\GFLORESC\\Documents\\connect.lisp")